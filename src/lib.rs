...
impl Sha1 {
    ...
    /// Initializes a sha1 hash object from a previously-computed digest (as you
    /// might use for a length extension attack).
    pub fn from_digest_bytes(digest: &[u8], len: u64) -> Sha1 {
        assert_eq!(digest.len(), 20);
        let state_vec: Vec<u32> = digest
            .chunks(4)
            .map(|bytes: &[u8]| {
            ...
            .collect::<Vec<u32>>();
        let state: [u32; 5] = <[u32; 5]>::try_from(&state_vec[..]).unwrap();
        Sha1 {
...
/*
 * XXX dap BEGIN copied from my cryptopals project
 */

/**
 * Given a byte buffer, print it out as a sequence of pairs of hex digits, each
 * pair representing a byte.
 */
#[allow(dead_code)]
fn bytes_to_hex(bytes: &[u8]) -> String {
    bytes_to_hex_by_block(bytes, bytes.len() + 1)
}

/**
 * Given a byte buffer, print it out as a sequence of pairs of hex digits, each
 * pair representing a byte.  Each "blocksize" bytes are printed on their own
 * line.
 */
fn bytes_to_hex_by_block(bytes: &[u8], blocksize: usize) -> String {
    let mut s = String::with_capacity((bytes.len() * 2) + bytes.len() / blocksize + 1);
    for (nbytes, byte) in bytes.iter().enumerate() {
        let high = (byte & 0xf0) >> 4;
        let highchar = nibble_to_hex(&high);
        let low = byte & 0xf;
        let lowchar = nibble_to_hex(&low);
        s.push(highchar);
        s.push(lowchar);
        if (nbytes + 1) % blocksize == 0 {
            s.push('\n');
        }
    }
    s
...
/**
 * Given a 4-bit value, return the ASCII code point representing this value in
 * hexadecimal.
 */
fn nibble_to_hex(n: &u8) -> char {
    ...
    char::try_from(cval as u32).unwrap()
}

/*
 * XXX dap END copied from my cryptopals project
 */
